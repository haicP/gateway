...
func (s *SQLiteStore) DeleteTracesBefore(ctx context.Context, cutoff time.Time) (int64, error) {
	if s == nil || s.db == nil {
		return 0, nil
	}
	...
	err := retrySQLiteBusy(ctx, func() error {
		res, err := s.db.ExecContext(ctx, `DELETE FROM traces WHERE timestamp < ?`, cutoff.UTC())
		if err != nil {
			return err
		}
		rowsAffected, err = res.RowsAffected()
		if err != nil {
			return fmt.Errorf("read deleted trace count: %w", err)
		}
		return nil
	})
	if err != nil {
