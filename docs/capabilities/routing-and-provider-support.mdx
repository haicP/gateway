...
description: Configure provider route prefixes, customize upstream endpoints, and verify prefix-based proxy forwarding.
related:
  title: Next steps
...
- Routes traffic by configured provider path prefixes.
...
- Supports OpenAI-compatible providers through `providers.llm.upstream`.
...
1. The gateway reads each entry under `providers` from config.
...
- Route OpenAI-compatible traffic through a compatible endpoint by setting
  `providers.llm.upstream`.
- Use environment-specific upstream hosts with `ONGOINGAI_LLM_UPSTREAM`.
- Place the default provider route under a shared ingress path such as `/ai/llm`.
...
- Cause: Prefix is missing a leading `/`, or upstream is missing scheme or
  host.
- Fix: Set valid values such as `prefix: /llm` and
  `upstream: https://api.openai.com`.
