...
- Full body backup capture:
  set `body_max_size: 0`; large Postgres bodies are stored outside the trace
  summary row and compressed asynchronously by the trace writer.
...
1. Send one proxied request through `/llm/...`.
...
- Trace detail responses may include `llm_response_content`, a structured JSON
  view of semantic model output. It is populated after the trace write and may
  be briefly absent immediately after a request completes.
...
- Cause: Requests did not go through provider routes, or upstream request failed
  before capture.
- Fix: Send traffic through `/llm/...`, then query traces
  again.
...
### `llm_response_content` is empty

- Symptom: Trace detail has no structured LLM output.
- Cause: The field is asynchronously derived from stored response bodies, so it
  can be briefly absent. It also stays empty when body capture is disabled,
  response body persistence is dropped by redaction, or large body redaction is
  skipped.
- Fix: Retry the detail read after a short delay and confirm response body
