...
- Emits structured JSON logs to `logging.dir` for request lifecycle and audit events.
  Logs include `trace_id` and `span_id` when an active span is in context,
  enabling direct log-to-trace correlation in Grafana Loki, Elasticsearch, or
  any log backend. Set `logging.stdout: true` to also mirror logs to stdout.
...
- OpenTelemetry spans and metrics use normalized request-scope attributes:
  `provider`, `model`, `org_id`, `workspace_id`, and `route`.
- When auth is enabled and identity is present, spans also include `key_id`
  and `role`.
- Proxy and provider latency histograms attach exemplars with `trace_id`,
  enabling click-through from dashboard latency spikes to the exact trace.
- Go runtime metrics (`go_memory_*`, `go_goroutine_*`, `go_gc_*`) are
  registered automatically for process health monitoring.
- Supports browser-based integration clients with CORS headers on API routes.

## Operational fit

- You need external usage, cost, latency, and error dashboards from gateway
  trace data.
...
4. `/api/analytics/*` exposes usage, cost, model, key, latency, error-rate, and
   summary aggregates.
5. Gateway logs are emitted as JSON lines in `logging.dir` and can be shipped
   by your log collector.
...
You can integrate by reading HTTP APIs and shipping log files.
...
- SIEM integration:
  ship JSON log files and index `audit_action`, `audit_outcome`, and `path`.
...
```bash filename="Terminal"
ONGOINGAI_LOG_STDOUT=true ongoingai serve | jq -c 'select(.audit_action != null)'
```
...
2. Send one proxied provider request through `/llm/...`.
...
- Structured JSON log lines in the configured log directory, or in stdout when
  `logging.stdout` is enabled.
...
- Fix: Configure the collector source as JSON line input for gateway log files,
  or enable `logging.stdout` when your collector reads process stdout.
