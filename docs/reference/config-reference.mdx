...
- `backup`: Scheduled request detail backup to S3-compatible object storage.
...
Each entry under `providers` uses the same schema. The default route is
`providers.llm`.

| Field | Type | Default | Required | Notes |
| --- | --- | --- | --- | --- |
| `providers.<provider>.upstream` | string | `https://api.openai.com` for `llm` | Yes | Must include scheme and host. |
| `providers.<provider>.prefix` | string | `/llm` for `llm` | Yes | Must start with `/`. Used for proxy path matching. |
...
| `tracing.body_max_size` | int | `1048576` | No | Max captured bytes per request and response body. Set to `0` or a negative value to capture without an application-level truncation limit. |
| `tracing.retention.days` | int | `14` | No | Number of days of traces to keep. Must be `> 0`. |
| `tracing.retention.cleanup_enabled` | bool | `false` | No | Enables the daily trace cleanup worker. Keep this `false` on all but one instance in multi-instance deployments. |
| `tracing.retention.cleanup_daily_at` | string | `02:00` | Conditional | Daily cleanup run time in `HH:MM` 24-hour format. Required when cleanup is enabled. |
| `tracing.retention.cleanup_timezone` | string | `Local` | Conditional | IANA timezone used to compute the daily cleanup run time. Required when cleanup is enabled. |

### `backup.request_details`

| Field | Type | Default | Required | Notes |
| --- | --- | --- | --- | --- |
| `backup.request_details.enabled` | bool | `false` | No | Enables the scheduled backup worker. Keep this `false` on all but one instance in multi-instance deployments. |
| `backup.request_details.timezone` | string | `Local` | Conditional | IANA timezone used to compute the previous-day window and daily run time. Required when enabled. |
| `backup.request_details.daily_at` | string | `02:00` | Conditional | Daily run time in `HH:MM` 24-hour format. Required when enabled. |
| `backup.request_details.temp_dir` | string | `./data/backup-tmp` | Conditional | Local directory for gzip NDJSON shard files before upload. Required when enabled. |
| `backup.request_details.page_size` | int | `500` | Conditional | Number of trace rows to read per export page. Must be `> 0` when enabled. |
| `backup.request_details.shard_max_bytes` | int64 | `209715200` | Conditional | Target max compressed shard size in bytes. Must be `> 0` when enabled. |
| `backup.request_details.retry.max_attempts` | int | `5` | Conditional | Max upload attempts per shard. Must be `> 0` when enabled. |
| `backup.request_details.retry.initial_backoff_ms` | int | `1000` | Conditional | Initial retry backoff in milliseconds. Must be `> 0` when enabled. |
| `backup.request_details.retry.max_backoff_ms` | int | `30000` | Conditional | Max retry backoff in milliseconds. Must be at least the initial backoff when enabled. |
| `backup.request_details.s3.bucket` | string | `""` | Conditional | Destination bucket. Required when enabled. Credentials use the AWS SDK default credential chain, not YAML. |
| `backup.request_details.s3.prefix` | string | `request-details` | No | Object key prefix. Backups are written under `prefix/YYMMDD/`. |
| `backup.request_details.s3.region` | string | `""` | No | AWS region override for the S3 client. |
| `backup.request_details.s3.endpoint` | string | `""` | No | S3-compatible endpoint URL. Must include scheme and host when set. |
| `backup.request_details.s3.force_path_style` | bool | `false` | No | Enables path-style addressing for S3-compatible stores such as MinIO. |

Request detail backup exports the previous local calendar day as gzip NDJSON
shards ordered by request start time. Each shard is uploaded only after it is
fully written to `temp_dir`; successful uploads delete the local shard, while
failed shards are retained after the configured finite retries.
...
| `observability.otel.metrics_enabled` | bool | `true` | Conditional | At least one of `traces_enabled`, `metrics_enabled`, or `prometheus_enabled` must be `true` when enabled. |
| `observability.otel.prometheus_enabled` | bool | `false` | No | Enables native Prometheus scrape endpoint. Can be combined with OTLP push metrics. |
| `observability.otel.prometheus_path` | string | `/metrics` | No | Path for the Prometheus scrape endpoint. Must start with `/` and must not overlap with `/api` or configured provider prefixes such as `/llm`. |
...
- In `redact_upstream` and `block`, provider-route request bodies are inspected
  synchronously and fail closed on policy-evaluation uncertainty. When
  `tracing.body_max_size<=0`, guardrail inspection still uses a conservative
  1MB cap.
...
| `providers.llm.upstream` | `ONGOINGAI_LLM_UPSTREAM` |
...
| `observability.otel.prometheus_enabled` | `ONGOINGAI_PROMETHEUS_ENABLED` |
| `observability.otel.prometheus_path` | `ONGOINGAI_PROMETHEUS_PATH` |
