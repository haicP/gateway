...
- Not included: provider proxy pass-through routes such as `/llm/...`.
...
- `response_headers`
- `response_body`
- `llm_response_content` (structured semantic LLM output; populated
  asynchronously when response body capture and privacy policy allow it)
- `metadata`
...
| `GET` | `/api/analytics/latency` | `analytics:read` when auth is enabled | `200` latency distribution stats |
| `GET` | `/api/analytics/errors` | `analytics:read` when auth is enabled | `200` error-rate breakdown stats |
...
Group options for `/api/analytics/latency` and `/api/analytics/errors`:

| Parameter | Type | Constraints | Notes |
| --- | --- | --- | --- |
| `group_by` | string | `provider`, `model`, `route`, `key`, or empty | Returns grouped breakdown rows. |
...
curl "http://localhost:8080/api/analytics/latency?group_by=route" \
  -H "X-OngoingAI-Gateway-Key: GATEWAY_KEY"

curl "http://localhost:8080/api/analytics/errors?group_by=key" \
  -H "X-OngoingAI-Gateway-Key: GATEWAY_KEY"
```
