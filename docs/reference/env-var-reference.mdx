...
| `ONGOINGAI_TRACE_RETENTION_DAYS` | int (`Atoi`) | `tracing.retention.days` | `ONGOINGAI_TRACE_RETENTION_DAYS=14` |
| `ONGOINGAI_TRACE_CLEANUP_ENABLED` | bool (`ParseBool`) | `tracing.retention.cleanup_enabled` | `ONGOINGAI_TRACE_CLEANUP_ENABLED=true` |
| `ONGOINGAI_TRACE_CLEANUP_DAILY_AT` | string (`HH:MM`) | `tracing.retention.cleanup_daily_at` | `ONGOINGAI_TRACE_CLEANUP_DAILY_AT=02:00` |
| `ONGOINGAI_TRACE_CLEANUP_TIMEZONE` | string | `tracing.retention.cleanup_timezone` | `ONGOINGAI_TRACE_CLEANUP_TIMEZONE=Asia/Shanghai` |
...
- `ONGOINGAI_PORT`, `ONGOINGAI_BODY_MAX_SIZE`, and `ONGOINGAI_TRACE_RETENTION_DAYS` must parse as integers.
- `ONGOINGAI_CAPTURE_BODIES`, `ONGOINGAI_AUTH_ENABLED`, trace cleanup bool fields, and backup bool fields must parse as booleans.
- Trace cleanup `DAILY_AT` must use `HH:MM`, and trace cleanup timezone must be a valid IANA timezone when cleanup is enabled.
