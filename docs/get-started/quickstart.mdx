...
- Stop the server and generate a local CLI report.
...
3. Set provider base URLs in Terminal B.
   ...
   - `ANTHROPIC_BASE_URL=http://localhost:8080/llm`
...
6. Stop the gateway in Terminal A (`Ctrl+C`), then generate a report in
   Terminal B.
   ...
   You should see totals plus a full lineage-chain debug view for the latest
   trace from local storage.
...
Run requests across two providers, then generate an offline report:
...
# Stop ongoingai serve in Terminal A first.
ongoingai report
ongoingai debug last
```

You should see request, token, and estimated cost totals plus the latest trace
chain checkpoints.
...
- Fix: Send a request to `/llm/...`, then query
  `/api/traces?limit=10`.
