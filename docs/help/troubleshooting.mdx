...
4. Send one proxied provider request:
   ...
   ```bash filename="Terminal"
   curl -i "http://localhost:8080/llm/v1/chat/completions" \
...
- Fix: Check `providers.<provider>.prefix`,
...
- Fix: Send traffic through `/llm/...`, then query
...
### `/api/analytics/*` returns `400` for query options

- Symptom: Usage, cost, latency, or error analytics request fails with query
  validation errors.
- Cause: Unsupported `group_by` or `bucket`, or invalid date range.
- Fix: For usage/cost, use `group_by=provider|model` and `bucket=hour|day|week`.
  For latency/errors, use `group_by=provider|model|route|key`. Ensure
  `to >= from`.
